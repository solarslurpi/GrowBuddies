...
def main():
    ...
    while "FAN" != fan_or_mister and "MIST" != fan_or_mister and "BOTH" != fan_or_mister:
    ...
    if "BOTH" == fan_or_mister:
        mqttAccess1 = growBuddy(subscribe_to_sensor=False, topic_key="mistBuddy_fan_topic", msg_value=on_or_off)
        mqttAccess1.start()

        mqttAccess2 = growBuddy(subscribe_to_sensor=False, topic_key="mistBuddy_fan_topic", msg_value=on_or_off)
        mqttAccess2.start()
        # Turn plugs on
        # Publish the mqtt message to turn the plugs on.
    else:
        mqttAccess1 = growBuddy(subscribe_to_sensor=False, topic_key=mqtt_topic_key, msg_value=on_or_off)
        mqttAccess1.start()
