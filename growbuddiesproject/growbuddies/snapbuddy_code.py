...
class SnapBuddy:
    def __init__(self, settings_string=None):
        ...
        if settings_string is None:
            self.logger.error(
                "An empty string was provided as an input for a valid entry in the growbuddies_settings for a snapbuddy. Please provide a valid string representing an entry in the growbuddies_settings for the snapbuddy."
            )
            sys.exit()
        settings = Settings()
        settings.load()
        ...
        if self.snapbuddy_settings_dict is None:
            self.logger.error(
                f"The settings_string '{settings_string}' was not found in the growbuddies_settings. Please provide a valid string representing an entry in the growbuddies_settings for the snapbuddy."
            )
            sys.exit()
        try:
            self.hostname = self.snapbuddy_settings_dict["hostname"]
            self.interval_secs = self.snapbuddy_settings_dict["interval_secs"]
            self.timelapse_dir = self.snapbuddy_settings_dict["timelapse_dir"]
        except KeyError as e:
            self.logger.error(
                f"The settings_string '{settings_string}' was found in the growbuddies_settings but the key '{e}' was not found. Please provide a valid {e} : value pair in the growbuddies_settings for the snapbuddy"
            )
            sys.exit()
    ...
    def _get_filename(self):
        ...
        subdir = os.path.dirname(self.timelapse_dir)
