...
def send_udp_message(msg: str) -> None:
    ...
    print(f"Sending to {host}:{port} message: {msg}")
    bmsg = bytes(msg, 'utf-8')
    try:
        if sock:
            sock.sendto(bmsg, (host,port) )  # send UDP packet to udp_host:port
...
def read_and_publish_scd4x_data(mqtt_client):
    ...
    while True:
        # need for subscription, but not publishing.  I believe it needs a lot more testing
        ...
        try:
            ...
            debug_print(f"Received exception trying to read and send scd4x data: {e}")
        time.sleep(3)  # Rest for a bit
...
if __name__ == "__main__":
    main()
 to turn of debug print statements.
