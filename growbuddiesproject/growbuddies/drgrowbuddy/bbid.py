        ...
        try:
            ...
            for index, link in enumerate(links):
                if limit is not None and len(tried_urls) >= limit:
                    exit(0)
                t = threading.Thread(
                    target=download, args=(pool_sema, img_sema, link, output_dir, limit)
                )
                t.start()
                current += 1
            last = links[-1]
...
def main():
    ...
    parser.add_argument(
        ...
        "--search-file",
        action="store_true",
        help="use search-string as a path to a file containing search strings line by line",
        required=False,
    )
    ...
    parser.add_argument(
        "-t", "--threads", help="Number of threads", type=int, default=20
    )
    ...
    if not args.search_file:
        ...
        for keyword in inputFile.readlines():
            ...
            fetch_images_from_keyword(
                pool_sema, img_sema, keyword, output_sub_dir, args.filters, args.limit
            )
            backup_history()
            time.sleep(10)
        inputFile.close()
