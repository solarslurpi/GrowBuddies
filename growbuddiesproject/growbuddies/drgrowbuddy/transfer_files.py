...
def transfer_files(
    src_dir: str = None, target_dir: str = None, size: tuple = (256, 256)
):
    """
    ...
    if existing_images:
        ...
        max_number = max(
            [int(re.search(r"(\d+)", img.name).group(1)) for img in existing_images]
        )
        # Get the md5 hash of the existing image files. This way, we can avoid copying duplicate files.
        existing_image_hashes = [
            hashlib.md5(img.read_bytes()).hexdigest() for img in existing_images
        ]
    ...
    for item in src_path.iterdir():
        if hashlib.md5(item.read_bytes()).hexdigest() in existing_image_hashes:
            print(
                f"{Fore.YELLOW}Skipping {item}.  The image already exists in the target directory.{Style.RESET_ALL}"
            )
            continue
        max_number += 1
        if item.is_dir():
            ...
            if mov_pattern.search(item.suffix):
                print(
                    f"{Fore.YELLOW}Skipping {item} because it is a movie file.{Style.RESET_ALL}"
                )
                continue
                ...
                with Image.open(item) as im:
                    ...
                    im.save(target_file)
                print(
                    f"{Fore.GREEN}Copying {item} to {target_file} and resized to {size}{Style.RESET_ALL}"
                )
            elif png_pattern.search(item.suffix) or webp_pattern.search(item.suffix):
                # Convert the png file to jpg
                with Image.open(item) as im:
                    rgb_img = (
                        Image.open(item).convert("RGB").resize(size, Image.LANCZOS)
                    )
                    rgb_img.save(target_file, "jpeg")
                print(
                    f"{Fore.BLUE}Converted {item} to {target_file} and resized to {size}{Style.RESET_ALL}"
                )
            else:
                print(
                    f"{Fore.RED}Unknown file type: {item}.  Skipping...{Style.RESET_ALL}"
                )
                continue
...
if __name__ == "__main__":
    transfer_files(
        src_dir="datasets/powdery_mildew_downloads",
        target_dir="datasets/powdery_mildew_256",
    )
