    ...
    with ZipFile(zip_file) as zf:
        ...
        for name in names:
            if not name.endswith(".jpg"):
                raise ValueError(
                    f"Error {name} does not end in .jpg. Was the zip file created with the Python script `transfer_files.py`"
                )
        ...
        categories = [
            ...
            and directory.startswith(base_directory + "/")
        ]
...
# ------------------------------------------------------------------------------------------------------
def pull_out_images(
    zip_filepath="drgb.zip",
    categories=["healthy", "powdery_mildew"],
    nImages_per_directory=10,
):
    # open the zip file
    filepaths = {}
    with ZipFile(zip_filepath, "r") as zip_ref:
        ...
        for category in categories:
            filepaths[category] = [
                filepath for filepath in all_filepaths if category in filepath
            ]
            filepaths[category] = random.sample(
                filepaths[category], nImages_per_directory
            )
